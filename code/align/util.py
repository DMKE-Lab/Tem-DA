...
def save_embeddings(folder, kgs, ent_embeds, rel_embeds, attr_embeds, mapping_mat=None):
    ...
    dict2file(folder + 'tkg1_ent_ids', kgs.tkg1.entities_id_dict)
    dict2file(folder + 'tkg2_ent_ids', kgs.tkg2.entities_id_dict)
    dict2file(folder + 'tkg1_rel_ids', kgs.tkg1.relations_id_dict)
    dict2file(folder + 'tkg2_rel_ids', kgs.tkg2.relations_id_dict)
    dict2file(folder + 'tkg1_attr_ids', kgs.tkg1.attributes_id_dict)
    dict2file(folder + 'tkg2_attr_ids', kgs.tkg2.attributes_id_dict)
...
def gat_load_data(input_folder, is_two=False, is_three=False, is_four=False):
    tkg1, tkg2, sup_ent1, sup_ent2, ref_ent1, ref_ent2, total_tri_num, total_e_num, total_r_num, rel_id_mapping = \
        read_dbp15k_input(input_folder)
    linked_ents = set(sup_ent1 + sup_ent2 + ref_ent1 + ref_ent2)
    enhanced_triples1, enhanced_triples2 = enhance_triples(tkg1, tkg2, sup_ent1, sup_ent2)
    ori_triples = tkg1.triple_list + tkg2.triple_list
    ...
    time1 = load_times(input_folder + 'triples_1')
    time2 = load_times(input_folder + 'triples_2')
    saved_data_path = input_folder + 'temda_saved_data.pkl'
    if os.path.exists(saved_data_path):
        ...
        if is_two:
            two_hop_triples1 = generate_2hop_triples(tkg1, linked_ents=linked_ents)
            two_hop_triples2 = generate_2hop_triples(tkg2, linked_ents=linked_ents)
            triples = two_hop_triples1 | two_hop_triples2
            two_adj, _ = no_weighted_adj(total_e_num, triples, is_two_adj=False)
            adj.append(two_adj)
        if is_three:
            three_hop_triples1 = generate_3hop_triples(tkg1, two_hop_triples1, linked_ents=linked_ents)
            three_hop_triples2 = generate_3hop_triples(tkg2, two_hop_triples2, linked_ents=linked_ents)
        ...
        if is_four:
            four_hop_triples1 = generate_3hop_triples(tkg1, three_hop_triples1, linked_ents=linked_ents)
            four_hop_triples2 = generate_3hop_triples(tkg2, three_hop_triples2, linked_ents=linked_ents)
            triples = four_hop_triples1 | four_hop_triples2
            four_adj, _ = no_weighted_adj(total_e_num, triples, is_two_adj=False)
            adj.append(four_adj)
        time_feature = get_time_matrix(triples, time1.union(time2))
        print('save adj data to', saved_data_path)
        pickle.dump(adj, open(saved_data_path, 'wb'))
        
    return adj, tkg1, tkg2, sup_ent1, sup_ent2, ref_ent1, ref_ent2, total_tri_num, \
           total_e_num, total_r_num, rel_id_mapping, rel_ht_dict, time_feature
...
def get_time_matrix(triples, time):
    ...
    return r_index, r_val, t_index, time_features

def rgcn_adj_list(tkg1, tkg2, adj_number, all_rel_num, all_ent_num):
    # *****************test two adj**********************************
    # adj_list = list()
    # adj1 = list()
    # adj2 = list()
    # for item in tkg1.triple_list:
    #     adj1.append([item[0], item[2]])
    #     adj1.append([item[2], item[0]])
    # for item in tkg2.triple_list:
    #     adj2.append([item[0], item[2]])
    #     adj2.append([item[2], item[0]])
    # tkg1_pos = np.array(adj1)
    # row, col = np.transpose(tkg1_pos)
    ...
    #
    # tkg2_pos = np.array(adj2)
    # row, col = np.transpose(tkg2_pos)
    # data = np.ones(row.shape[0])
    # adj = sp.coo_matrix((data, (row, col)), shape=(all_ent_num, all_ent_num))
    # # adj = preprocess_adj(adj)
    # adj = diag_adj(adj)
    ...
    adj_list = list()
    triple_list = tkg1.triple_list + tkg2.triple_list
    edge = dict()
...
def test_no_weighted_adj(total_ent_num, tkg1_triple_list, tkg2_triple_list):
    adj = list()
    for triple_list in [tkg1_triple_list, tkg2_triple_list]:
        edge = dict()
        for item in triple_list:
            if 0 <= item[0] < 10500:
                item_first = item[0]
...
def relation_adj_list(tkg1, tkg2, adj_number, all_rel_num, all_ent_num, linked_ents, rel_id_mapping):
...
def transloss_add2hop(tkg1, tkg2, sup_ent1, sup_ent2, ref_ent1, ref_ent2, total_e_num):
    linked_ents = set(sup_ent1 + sup_ent2 + ref_ent1 + ref_ent2)
    enhanced_triples1 = generate_2hop_triples(tkg1, linked_ents=linked_ents)
    enhanced_triples2 = generate_2hop_triples(tkg2, linked_ents=linked_ents)
